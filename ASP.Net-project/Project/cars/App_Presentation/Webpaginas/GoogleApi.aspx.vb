...
Partial Class App_Presentation_Webpaginas_Default2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Centreren op filiaal opgeslagen in cookie

        Dim FiliaalCookie As HttpCookie
        FiliaalCookie = Request.Cookies("filcookie")

        If Not FiliaalCookie Is Nothing Then
            GeoCode = GMap.geoCodeRequest(FiliaalCookie.Value, MapKey)
        ...
        For Each dr In tblFil.Rows
            ...
            ' ervoor zorgen dat de infowindow van cookiefiliaal wordt weergegeven
            If StrComp(Adres, FiliaalCookie.Value) = 0 Then
                window = New GInfoWindow(marker, "<h3>" & Adres & "</h3> <hr /> <br /><br /> <hr /> <input type='button' value='Bepaal filiaal' onClick= & test() & />", True)
            Else
                window = New GInfoWindow(marker, "<h3>" & Adres & "</h3> <hr /> <br /><br /> <hr /> <input type='button' value='Bepaal filiaal' onClick= & test() & />", False)
            End If

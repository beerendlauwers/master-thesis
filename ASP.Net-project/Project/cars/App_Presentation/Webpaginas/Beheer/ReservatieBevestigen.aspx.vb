...
Partial Class App_Presentation_Webpaginas_ReservatieBevestigen
    ...
    Private bllKlant As New KlantBLL
    ...
    Protected Sub btnBevestigen_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBevestigen.Click
        ...
        Try
            ...
            Dim dt As Klanten.tblUserProfielDataTable = bllKlant.GetUserProfielByNaamEnVoornaam(lblNaam.Text, lblVoornaam.Text)
    ...
    Protected Sub btnZoek_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnZoekGebruiker.Click
        lblNaam.Text = Nothing
        lblVoornaam.Text = Nothing
        Dim dt As New Klanten.tblUserProfielDataTable
        Dim userID As New Guid
        Dim usernaam As String = txtGebruikersnaam.Text
        Try
            userID = New Guid(Membership.GetUser(usernaam).ProviderUserKey.ToString())
            dt = bllKlant.GetUserProfielByUserID(userID)
            lblVoornaam.Text = dt.Rows(0)("userVoornaam")
            lblNaam.Text = dt.Rows(0)("userNaam")
            lblGegevens.Visible = True
        Catch ex As Exception
            'Throw ex
            lblNaam.Text = "U hebt geen geldige gebruikersnaam opgegeven."
        End Try

    End Sub

    Protected Sub btnZoekIdentiteit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnZoekIdentiteit.Click
        lblNaam.Text = Nothing
        lblVoornaam.Text = Nothing
        Dim dt As New Klanten.tblUserProfielDataTable
        If bllKlant.GetUserProfielByIdentiteitskaartNr(txtIdentiteit.Text).Rows.Count > 0 Then
            dt = bllKlant.GetUserProfielByIdentiteitskaartNr(txtIdentiteit.Text)
            lblVoornaam.Text = dt.Rows(0)("userVoornaam")
            lblNaam.Text = dt.Rows(0)("userNaam")
            lblGegevens.Visible = True
        Else
            lblNaam.Text = "u hebt geen geldig identiteitsnummer opgegeven."
        End If
    End Sub

    Protected Sub btnZoekRijbewijs_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnZoekRijbewijs.Click
        lblNaam.Text = Nothing
        lblVoornaam.Text = Nothing
        Dim dt As New Klanten.tblUserProfielDataTable
        If bllKlant.GetUserProfielByRijbewijs(txtRijbewijs.Text).Rows.Count > 0 Then
            dt = bllKlant.GetUserProfielByRijbewijs(txtRijbewijs.Text)
            lblVoornaam.Text = dt.Rows(0)("userVoornaam")
            lblNaam.Text = dt.Rows(0)("userNaam")
            lblGegevens.Visible = True
        Else
            lblNaam.Text = "U hebt geen geldig RijbewijsNr opgegeven"
        End If

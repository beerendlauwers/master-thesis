...
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = dojo.version){
...
var _a=_6.split(".");
var _b=_a.pop();
for(var i=0,l=DanaGetLength(_a);i<l&&_9;i++){
...
return dj_global.eval?dj_global.eval(_15):eval(DanaEval(_15));
}
...
for(var i=0;i<DanaGetLength(_1e);i++){
tmp[_1e[i]]=false;
}
return tmp;
}
...
if(DanaGetLength(djConfig.baseScriptUri)){
return djConfig.baseScriptUri;
}
var uri=new String(DanaStringCheckLink(djConfig.libraryScriptUri||djConfig.baseRelativePath));
if(!uri){
...
uri+="?"+DanaMethodReplace("replace",String(DanaStringCheckLink(djConfig.cacheBust)),/\W+/g,"");
}
try{
...
for(var x=0;x<DanaGetLength(mll);x++){
mll[x]();
}
...
if(DanaGetLength(this.loadUriStack)==0&&DanaGetLength(this.getTextStack)==0){
...
DanaFuncSetTimeout(setTimeout,"dojo.hostenv.loaded();",0);
}else{
dojo.hostenv.loaded();
}
};
...
var _43=_42.split(".");
for(var i=DanaGetLength(_43);i>0;i--){
...
var _4b=DanaMethodReplace("replace",_47,/\./g,"/")+".js";
var _4c=_47.split(".");
var _4d=this.getModuleSymbols(_47);
var _4e=((_4d[0].charAt(0)!="/")&&!_4d[0].match(/^\w+:/));
var _4f=_4d[DanaGetLength(_4d)-1];
var ok;
if(_4f=="*"){
_47=_4c.slice(0,-1).join(".");
while(DanaGetLength(_4d)){
_4d.pop();
...
var _53=String(DanaStringCheckLink(_52));
var _54=_53;
var _55=_52.split(/\./);
if(_55[DanaGetLength(_55)-1]=="*"){
_55.pop();
_54=_55.join(".");
}
...
var lmn=String(DanaStringCheckLink(_57));
...
for(var x=0;x<DanaGetLength(_5c);x++){
var _5e=_5c[x];
...
var _63=[];
for(var i=1;i<DanaGetLength(arguments);i++){
...
var p=_6b.split(".");
for(var i=0;i<DanaGetLength(p);i++){
if(!obj[p[i]]){
return false;
}
obj=obj[p[i]];
...
var _73=_70.split("-");
var _74=[];
for(var i=DanaGetLength(_73);i>0;i--){
...
for(var j=DanaGetLength(_74)-1;j>=0;j--){
var loc=_74[j]||"ROOT";
var _78=_72(loc);
if(_78){
break;
...
for(var i=0;i<DanaGetLength(dojo.hostenv.localesGenerated);i++){
...
for(var i=0;i<DanaGetLength(_7c);i++){
preload(_7c[i]);
}
}
dojo.hostenv.preloadLocalizations=function(){
...
if(_81){
var _85=_81.split(",");
for(var i=0;i<DanaGetLength(_85);i++){
if(_82.indexOf(_85[i])==0){
if(DanaGetLength(_85[i])>DanaGetLength(_84)){
_84=_85[i];
}
}
}
...
var _8a=DanaMethodReplace("replace",_87,"-","_");
var _8b=_83+"."+_8a;
...
var _90=DanaMethodReplace("replace",loc,"-","_");
var _91=_83+"."+_90;
var _92=false;
...
if(_81&&_82!=_84){
_88[DanaMethodReplace("replace",_82,"-","_")]=_88[DanaMethodReplace("replace",_84,"-","_")];
}
};
(function(){
...
for(var i=0;i<DanaGetLength(_98);i++){
req(m,b,_98[i],_9d);
}
};
}
...
var _9f=DanaMethodToString("toString",document.location);
var _a0=_9f.split("?",2);
if(DanaGetLength(_a0)>1){
var _a1=_a0[1];
var _a2=_a1.split("&");
for(var x in _a2){
var sp=_a2[x].split("=");
if((DanaGetLength(sp[0])>9)&&(sp[0].substr(0,9)=="djConfig.")){
var opt=sp[0].substr(9);
try{
djConfig[opt]=eval(DanaEval(sp[1]));
}
catch(e){
djConfig[opt]=sp[1];
}
...
for(var i=0;i<DanaGetLength(_a6);i++){
var src=DanaMethodGetAttribute("getAttribute",_a6[i],"src");
if(!src){
continue;
}
var m=src.match(_a7);
...
_c4=DanaMethodSetTimeout("setTimeout",gbl,function(){
...
DanaMethodOpen("open",_c0,"GET",uri,_be?true:false);
try{
_c0.send(null);
if(_be){
return null;
...
var div=DanaMethodCreateElement("createElement",document,"div");
DanaMethodAppendChild("appendChild",div,document.createTextNode(_c8));
DanaMethodAppendChild("appendChild",_c9,div);
}
catch(e){
try{
DanaMethodWrite("write",document,"<div>"+_c8+"</div>");
}
catch(e2){
window.status=_c8;
}
...
while(DanaGetLength(dojo.hostenv._println_buffer)>0){
...
var _timer=DanaFuncSetInterval(setInterval,function(){
...
DanaMethodSetTimeout("setTimeout",window,function(){
...
var _d3=[];
if(djConfig.searchIds&&DanaGetLength(djConfig.searchIds)>0){
_d3=_d3.concat(djConfig.searchIds);
}
if(dojo.hostenv.searchIds&&DanaGetLength(dojo.hostenv.searchIds)>0){
_d3=_d3.concat(dojo.hostenv.searchIds);
}
if((djConfig.parseWidgets)||(DanaGetLength(_d3)>0)){
if(dojo.evalObjPath("dojo.widget.Parse")){
var _d4=new dojo.xml.Parse();
if(DanaGetLength(_d3)>0){
for(var x=0;x<DanaGetLength(_d3);x++){
...
DanaMethodAddRule("addRule",DanaMethodCreateStyleSheet("createStyleSheet",document),"v\\:*","behavior:url(#default#VML)");
}
}
catch(e){
}
...
eles=doc.all[id];
if(eles){
if(DanaGetLength(eles)){
for(var i=0;i<DanaGetLength(eles);i++){
if(eles[i].id==id){
ele=eles[i];
break;
}
...
if(!DanaGetLength(str)){
return str;
}
var re=(wh>0)?(/^\s+/):(wh<0)?(/\s+$/):(/^\s+|\s+$/g);
return DanaMethodReplace("replace",str,re,"");
};
...
var out=String(DanaStringCheckLink(str));
if(!c){
c="0";
}
if(!dir){
...
for(var i=1,l=DanaGetLength(arguments);i<l;i++){
...
if(_114){
var step=-1;
var i=DanaGetLength(_111)-1;
var end=-1;
}else{
var step=1;
var i=0;
var end=DanaGetLength(_111);
}
if(_113){
while(i!=end){
if(_111[i]===_112){
...
if(dojo.lang.isNumber(DanaGetLength(it))&&isFinite(DanaGetLength(it))){
return true;
}
return false;
};
...
return !dojo.lang.isFunction(it)&&/\{\s*\[native code\]\s*\}/.test(String(DanaStringCheckLink(it)));
};
...
var args=[];
for(var i=_12c;i<DanaGetLength(arguments);i++){
args.push(arguments[i]);
}
return DanaMethodSetTimeout("setTimeout",dojo.global(),function(){
...
for(var x in ns){
if(ns[x]===item){
return new String(DanaStringCheckLink(x));
}
}
return null;
};
...
for(var i=0;i<DanaGetLength(arguments);i++){
...
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = dojo.parseObjPath(_138,_139,_13a)){
return dojo.evalProp(prop,obj,_13a);
}}
};
dojo.lang.setObjPathValue=function(_13b,_13c,_13d,_13e){
dojo.deprecated("dojo.lang.setObjPathValue","use dojo.parseObjPath and the '=' operator","0.6");
if(DanaGetLength(arguments)<4){
_13e=true;
}
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = dojo.parseObjPath(_13b,_13d,_13e)){
...
if((DanaGetLength(arguments)==1)&&(arguments[0].constructor==Object)){
...
if(_156["url"]){
_156.url=DanaMethodToString("toString",_156.url);
}
if(_156["formNode"]){
...
for(var x=0;x<DanaGetLength(dojo.io.hdlrFuncNames);x++){
...
for(var x=0;x<DanaGetLength(dojo.io.transports);x++){
...
if(DanaGetLength(dojo.io._bindQueue)>0){
...
if(!_177){
if(r.safari){
DanaPutLocation(_175,src,0);
}else{
DanaPutLocation(frames[_175.name],src,0);
}
}else{
var idoc;
if(r.ie){
...
DanaFuncLocationReplace(idoc,src);
}
}
}
catch(e){
...
var _185=[];
for(var i=0;i<DanaGetLength(arr);++i){
...
var _18b=_188;
if(DanaGetLength(arguments)==2){
_18a=_188;
_18b=arr[0];
arr=arr.slice(1);
}else{
if(DanaGetLength(arguments)==3){
...
for(var i=0,l=DanaGetLength(_18f);i<l;i++){
...
for(var i=0,l=DanaGetLength(arr);i<l;i++){
...
var _1ac=[];
for(var i=_1ab||0;i<DanaGetLength(_1aa);i++){
_1ac.push(_1aa[i]);
}
return _1ac;
}});
...
var args=[];
for(var x=2;x<DanaGetLength(arguments);x++){
...
return function(){
var ta=args.concat([]);
for(var x=0;x<DanaGetLength(arguments);x++){
ta.push(arguments[x]);
}
...
var _1c0=(func["__preJoinArity"]||DanaGetLength(func))-DanaGetLength(_1be);
function gather(_1c1,_1c2,_1c3){
var _1c4=_1c3;
var _1c5=_1c2.slice(0);
for(var x=0;x<DanaGetLength(_1c1);x++){
_1c5.push(_1c1[x]);
}
_1c3=_1c3-DanaGetLength(_1c1);
if(_1c3<=0){
...
var _1cc=[];
var x=_1cb||0;
for(x=_1cb;x<DanaGetLength(args);x++){
_1cc.push(args[x]);
}
...
return DanaMethodReplace("replace",_1d4,/\%\{(\w+)\}/g,function(_1d7,key){
...
if(DanaGetLength(arguments)==0){
str=this;
}
var _1da=str.split(" ");
for(var i=0;i<DanaGetLength(_1da);i++){
...
return (DanaGetLength(dojo.string.trim(str))==0);
};
...
_1df=_1df.substring(_1e0.index+DanaGetLength(_1e0[0]));
}
ret+=DanaMethodReplace("replace",_1df,/\+/g,"%2B");
return ret;
};
...
str=DanaMethodReplace("replace",DanaMethodReplace("replace",DanaMethodReplace("replace",DanaMethodReplace("replace",str,/&/gm,"&amp;"),/</gm,"&lt;"),/>/gm,"&gt;"),/"/gm,"&quot;");
if(!_1e8){
str=DanaMethodReplace("replace",str,/'/gm,"&#39;");
}
return str;
};
dojo.string.escapeSql=function(str){
return DanaMethodReplace("replace",str,/'/gm,"''");
};
dojo.string.escapeRegExp=function(str){
return DanaMethodReplace("replace",DanaMethodReplace("replace",str,/\\/gm,"\\\\"),/([\f\b\n\t\r[\^$|?*+(){}])/gm,"\\$1");
};
dojo.string.escapeJavaScript=function(str){
return DanaMethodReplace("replace",str,/(["'\f\b\n\t\r])/gm,"\\$1");
};
dojo.string.escapeString=function(str){
return DanaMethodReplace("replace",DanaMethodReplace("replace",DanaMethodReplace("replace",DanaMethodReplace("replace",DanaMethodReplace("replace",("\""+DanaMethodReplace("replace",str,/(["\\])/g,"\\$1")+"\""),/[\f]/g,"\\f"),/[\b]/g,"\\b"),/[\n]/g,"\\n"),/[\t]/g,"\\t"),/[\r]/g,"\\r");
};
dojo.string.summary=function(str,len){
if(!len||DanaGetLength(str)<=len){
return str;
}
return DanaMethodReplace("replace",str.substring(0,len),/\.+$/,"")+"...";
};
...
if((DanaGetLength(str)-DanaGetLength(end))<0){
return false;
}
return str.lastIndexOf(end)==DanaGetLength(str)-DanaGetLength(end);
};
...
if(_1fc=="\n"){
text=DanaMethodReplace("replace",text,/\r\n/g,"\n");
text=DanaMethodReplace("replace",text,/\r/g,"\n");
}else{
if(_1fc=="\r"){
text=DanaMethodReplace("replace",text,/\r\n/g,"\r");
text=DanaMethodReplace("replace",text,/\n/g,"\r");
}else{
text=DanaMethodReplace("replace",DanaMethodReplace("replace",text,/([^\r])\n/g,"$1\r\n"),/\r([^\n])/g,"\r\n$1");
}
}
return text;
};
...
var _1ff=[];
for(var i=0,_201=0;i<DanaGetLength(str);i++){
if(str.charAt(i)=="\\"){
i++;
continue;
}
...
var node=DanaGetFirstChild(_205);
...
var _212=0;
if(trim){
while(_20f.hasChildNodes()&&DanaGetFirstChild(_20f).nodeType==dojo.dom.TEXT_NODE){
_20f.removeChild(DanaGetFirstChild(_20f));
}
...
DanaMethodAppendChild("appendChild",_210,DanaGetFirstChild(_20f));
_212++;
}
return _212;
};
...
var _219=[];
if(dojo.render.html.ie){
for(var i=0;i<DanaGetLength(DanaGetChildNodes(node));i++){
_219.push(DanaGetChildNodes(node)[i]);
}
}
dojo.dom.removeChildren(node);
DanaMethodAppendChild("appendChild",node,_218);
for(var i=0;i<DanaGetLength(_219);i++){
dojo.dom.destroyNode(_219[i]);
}
};
dojo.dom.removeChildren=function(node){
var _21c=DanaGetLength(DanaGetChildNodes(node));
while(node.hasChildNodes()){
dojo.dom.removeNode(DanaGetFirstChild(node));
}
return _21c;
};
...
for(var i=0;i<DanaGetLength(_232);i++){
try{
...
var tmp=DanaMethodCreateElement("createElement",_238,"xml");
DanaPutInnerHTML(tmp,str,0);
if(_238.implementation&&_238.implementation.createDocument){
var _23a=_238.implementation.createDocument("foo","",null);
for(var i=0;i<DanaGetLength(DanaGetChildNodes(tmp));i++){
_23a.importNode(DanaMethodItem("item",DanaGetChildNodes(tmp),i),true);
}
return _23a;
}
return ((tmp.document)&&(DanaGetFirstChild(tmp.document)?DanaGetFirstChild(tmp.document):tmp));
}
}
}
return null;
...
if(DanaGetFirstChild(_23d)){
_23d.insertBefore(node,DanaGetFirstChild(_23d));
}else{
DanaMethodAppendChild("appendChild",_23d,node);
}
return true;
};
...
DanaMethodAppendChild("appendChild",pn,node);
}else{
...
case "first":
if(DanaGetFirstChild(ref)){
return dojo.dom.insertBefore(node,DanaGetFirstChild(ref));
}else{
DanaMethodAppendChild("appendChild",ref,node);
return true;
}
break;
default:
...
var _24c=DanaGetChildNodes(_24a);
if(!DanaGetLength(_24c)||DanaGetLength(_24c)==_24b){
DanaMethodAppendChild("appendChild",_24a,node);
return true;
}
if(_24b==0){
...
for(var i=0;i<DanaGetLength(DanaGetChildNodes(node));i++){
switch(DanaGetChildNodes(node)[i].nodeType){
case 1:
case 5:
_250+=dojo.dom.textContent(DanaGetChildNodes(node)[i]);
break;
case 3:
case 2:
case 4:
_250+=DanaGetChildNodes(node)[i].nodeValue;
break;
default:
break;
}
...
for(var i=1;i<DanaGetLength(arguments);i++){
if(node.tagName==String(DanaStringCheckLink(arguments[i]))){
return String(DanaStringCheckLink(arguments[i]));
}
}
}
return "";
...
DanaMethodSetAttributeNS("setAttributeNS",elem,_256,_257,_258);
}else{
...
DanaMethodWrite("write",document,"<iframe style='border: 0px; width: 1px; height: 1px; position: absolute; bottom: 0px; right: 0px; visibility: visible;' name='djhistory' id='djhistory' src='"+(djConfig["dojoIframeHistoryUrl"]||dojo.hostenv.getBaseScriptUri()+"iframe_history.html")+"'></iframe>");
}
}
catch(e){
}
...
dojo.undo.browser={initialHref:(!dj_undef("window"))?DanaGetHref(window.location):"",initialHash:(!dj_undef("window"))?window.location.hash:"",moveForward:false,historyStack:[],forwardStack:[],historyIframe:null,bookmarkAnchor:null,locationTimer:null,setInitialState:function(args){
...
this.historyIframe=DanaGetFrames(window)["djhistory"];
}
if(!this.bookmarkAnchor){
this.bookmarkAnchor=DanaMethodCreateElement("createElement",document,"a");
DanaMethodAppendChild("appendChild",dojo.body(),this.bookmarkAnchor);
this.bookmarkAnchor.style.display="none";
}
if(args["changeUrl"]){
hash="#"+((args["changeUrl"]!==true)?args["changeUrl"]:(new Date()).getTime());
if(DanaGetLength(this.historyStack)==0&&this.initialState.urlHash==hash){
this.initialState=this._createState(url,args,hash);
return;
}else{
if(DanaGetLength(this.historyStack)>0&&this.historyStack[DanaGetLength(this.historyStack)-1].urlHash==hash){
this.historyStack[DanaGetLength(this.historyStack)-1]=this._createState(url,args,hash);
return;
}
}
this.changingUrl=true;
DanaFuncSetTimeout(setTimeout,"window.location.href = '"+hash+"'; dojo.undo.browser.changingUrl = false;",1);
DanaPutHref(this.bookmarkAnchor,hash,0);
if(dojo.render.html.ie){
url=this._loadIframeHistory();
var _25f=args["back"]||args["backButton"]||args["handle"];
var tcb=function(_261){
if(window.location.hash!=""){
DanaFuncSetTimeout(setTimeout,"window.location.href = '"+hash+"';",1);
}
...
if(window.location.hash!=""){
DanaPutHref(window.location,hash,0);
}
if(_262){
...
this.locationTimer=DanaFuncSetInterval(setInterval,"dojo.undo.browser.checkLocation();",200);
}
}
}
}else{
...
if(!this.changingUrl){
var hsl=DanaGetLength(this.historyStack);
if((window.location.hash==this.initialHash||DanaGetHref(window.location)==this.initialHref)&&(hsl==1)){
this.handleBackButton();
return;
}
if(DanaGetLength(this.forwardStack)>0){
if(this.forwardStack[DanaGetLength(this.forwardStack)-1].urlHash==window.location.hash){
...
if(!dojo.render.html.opera){
var _268=this._getUrlQuery(DanaGetHref(_267));
if(_268==null){
if(DanaGetLength(this.historyStack)==1){
this.handleBackButton();
}
return;
}
...
if(DanaGetLength(this.historyStack)>=2&&_268==this._getUrlQuery(this.historyStack[DanaGetLength(this.historyStack)-2].url)){
this.handleBackButton();
}else{
if(DanaGetLength(this.forwardStack)>0&&_268==this._getUrlQuery(this.forwardStack[DanaGetLength(this.forwardStack)-1].url)){
this.handleForwardButton();
}
}
}
...
var last=this.historyStack[DanaGetLength(this.historyStack)-1];
if(!last&&DanaGetLength(this.historyStack)==0){
...
if(DanaMethodGetAttribute("getAttribute",_275,"type")=="file"){
_273=true;
}
});
return _273;
...
while(DanaGetFirstChild(node)){
dojo.dom.destroyNode(DanaGetFirstChild(node));
}
DanaPutInnerHTML(node,d,0);
};
dojo.io.bind(args);
};
...
var _283=[];
for(var i=0;i<DanaGetLength(_27f.elements);i++){
...
for(var j=0;j<DanaGetLength(elm.options);j++){
...
for(var i=0;i<DanaGetLength(_289);i++){
var _28a=_289[i];
...
for(var i=0;i<DanaGetLength(form.elements);i++){
...
for(var i=0;i<DanaGetLength(_290);i++){
var _291=_290[i];
...
if(((http.status>=200)&&(http.status<300))||(http.status==304)||(http.status==1223)||(DanaGetProtocol(location)=="file:"&&(http.status==0||http.status==undefined))||(DanaGetProtocol(location)=="chrome:"&&(http.status==0||http.status==undefined))){
var ret;
...
var _2b1=_2b0.split(/[\r\n]+/g);
for(var i=0;i<DanaGetLength(_2b1);i++){
...
if(!ret||typeof ret=="string"||!DanaMethodGetResponseHeader("getResponseHeader",http,"Content-Type")){
...
this.inFlightTimer=DanaFuncSetTimeout(setTimeout,"dojo.io.XMLHTTPTransport.watchInFlight();",10);
}
};
this.watchInFlight=function(){
var now=null;
if(!dojo.hostenv._blockAsync&&!_29e._blockAsync){
for(var x=DanaGetLength(this.inFlight)-1;x>=0;x--){
try{
...
if(DanaGetLength(this.inFlight)==0){
this.inFlightTimer=null;
return;
}
this.inFlightTimer=DanaFuncSetTimeout(setTimeout,"dojo.io.XMLHTTPTransport.watchInFlight();",10);
...
var _2c1="";
if(_2bf["formNode"]){
var ta=DanaMethodGetAttribute("getAttribute",_2bf.formNode,"action");
if((ta)&&(!_2bf["url"])){
url=ta;
}
var tp=DanaMethodGetAttribute("getAttribute",_2bf.formNode,"method");
if((tp)&&(!_2bf["method"])){
_2bf.method=tp;
}
...
var t=[];
if(DanaGetLength(_2c1)){
var q=_2c1.split("&");
for(var i=0;i<DanaGetLength(q);++i){
if(DanaGetLength(q[i])){
var p=q[i].split("=");
...
for(var i=0;i<DanaGetLength(_2bf.file);++i){
var o=_2bf.file[i];
...
if(!_2bf.user){
DanaMethodOpen("open",http,"POST",url,_2ca);
}else{
DanaMethodOpen("open",http,"POST",url,_2ca,_2bf.user,_2bf.password);
}
...
if(!_2bf.user){
DanaMethodOpen("open",http,_2bf.method.toUpperCase(),_2d1,_2ca);
}else{
DanaMethodOpen("open",http,_2bf.method.toUpperCase(),_2d1,_2ca,_2bf.user,_2bf.password);
}
setHeaders(http,_2bf);
try{
http.send(null);
...
DanaGetCookie(dojo.io).setCookie=function(name,_2d3,days,path,_2d6,_2d7){
var _2d8=-1;
...
_2d3=escape(_2d3);
DanaPutCookie(document,name+"="+_2d3+";"+(_2d8!=-1?" expires="+_2d8+";":"")+(path?"path="+path:"")+(_2d6?"; domain="+_2d6:"")+(_2d7?"; secure":""),0);
};
DanaGetCookie(dojo.io).set=DanaGetCookie(dojo.io).setCookie;
DanaGetCookie(dojo.io).getCookie=function(name){
var idx=DanaGetCookie(document).lastIndexOf(name+"=");
if(idx==-1){
return null;
}
var _2dc=DanaGetCookie(document).substring(idx+DanaGetLength(name)+1);
var end=_2dc.indexOf(";");
if(end==-1){
end=DanaGetLength(_2dc);
}
_2dc=_2dc.substring(0,end);
_2dc=unescape(_2dc);
return _2dc;
};
DanaGetCookie(dojo.io).get=DanaGetCookie(dojo.io).getCookie;
DanaGetCookie(dojo.io).deleteCookie=function(name){
DanaGetCookie(dojo.io).setCookie(name,"-",0);
};
DanaGetCookie(dojo.io).setObjectCookie=function(name,obj,days,path,_2e3,_2e4,_2e5){
if(DanaGetLength(arguments)==5){
_2e5=_2e3;
_2e3=null;
_2e4=null;
}
var _2e6=[],_2e7,_2e8="";
if(!_2e5){
_2e7=DanaGetCookie(dojo.io).getObjectCookie(name);
}
if(days>=0){
if(!_2e7){
_2e7={};
...
DanaGetCookie(dojo.io).setCookie(name,_2e8,days,path,_2e3,_2e4);
};
DanaGetCookie(dojo.io).getObjectCookie=function(name){
var _2eb=null,_2ec=DanaGetCookie(dojo.io).getCookie(name);
if(_2ec){
_2eb={};
var _2ed=_2ec.split("&");
for(var i=0;i<DanaGetLength(_2ed);i++){
...
DanaGetCookie(dojo.io).isSupported=function(){
if(typeof navigator.cookieEnabled!="boolean"){
DanaGetCookie(dojo.io).setCookie("__TestingYourBrowserForCookieSupport__","CookiesAllowed",90,null);
var _2f1=DanaGetCookie(dojo.io).getCookie("__TestingYourBrowserForCookieSupport__");
...
if(!dojo.io.cookies){
dojo.io.cookies=DanaGetCookie(dojo.io);
}
...
var dl=dojo.lang;
var ao={srcObj:dj_global,srcFunc:null,adviceObj:dj_global,adviceFunc:null,aroundObj:null,aroundFunc:null,adviceType:(DanaGetLength(args)>2)?args[0]:"after",precedence:"last",once:false,delay:null,rate:0,adviceMsg:false,maxCalls:-1};
switch(DanaGetLength(args)){
case 0:
return;
case 1:
return;
...
var _300;
if((DanaGetLength(arguments)==1)&&(typeof a1=="object")){
_300=a1;
}else{
...
this.object=obj;
this.args=[];
for(var x=0;x<DanaGetLength(args);x++){
this.args[x]=args[x];
}
this.around_index=-1;
};
...
this.around_index++;
if(this.around_index>=DanaGetLength(this.jp_.around)){
...
marr.delayTimer=DanaFuncSetTimeout(setTimeout,function(){
mcpy[5]=0;
_329(mcpy);
},tod);
}
...
if((this["before"])&&(DanaGetLength(this.before)>0)){
dojo.lang.forEach(this.before.concat(new Array()),_33a);
}
var _33b;
try{
if((this["around"])&&(DanaGetLength(this.around)>0)){
...
if((this["after"])&&(DanaGetLength(this.after)>0)){
...
var ind=-1;
for(var x=0;x<DanaGetLength(arr);x++){
var aao=(typeof _34e=="object")?DanaMethodToString("toString",(new String(DanaStringCheckLink(_34e)))):_34e;
var a1o=(typeof arr[x][1]=="object")?DanaMethodToString("toString",(new String(DanaStringCheckLink(arr[x][1])))):arr[x][1];
...
var args=[];
for(var x=1;x<DanaGetLength(arguments);x++){
args.push(arguments[x]);
}
...
na=[_37b];
for(var x=0;x<DanaGetLength(tna);x++){
...
na=(DanaGetLength(this.clobberNodes))?this.clobberNodes:document.all;
}
tna=null;
var _37f={};
for(var i=DanaGetLength(na)-1;i>=0;i=i-1){
var el=na[i];
try{
if(el&&el["__clobberAttrs__"]){
for(var j=0;j<DanaGetLength(el.__clobberAttrs__);j++){
...
for(var x=0;x<DanaGetLength(_38a);x++){
...
hex=hex.toUpperCase();
if(DanaMethodReplace("replace",hex,new RegExp("["+_3bd+"]","g"),"")!=""){
return null;
}
if(DanaGetLength(hex)==3){
...
for(var i=0;i<DanaGetLength(rgb);i++){
...
x=new Number(x);
var s=DanaMethodToString("toString",x,16);
while(DanaGetLength(s)<2){
s="0"+s;
}
return s;
});
...
if(_3e4>0){
DanaFuncSetTimeout(setTimeout,dojo.lang.hitch(this,function(){
this.play(null,_3e5);
}),_3e4);
return this;
}
...
if(step<1){
this._timer=DanaFuncSetTimeout(setTimeout,dojo.lang.hitch(this,"_cycle"),this.rate);
}else{
this._active=false;
...
this._animsEnded=0;
var _3f2=arguments;
if(DanaGetLength(_3f2)==1&&(dojo.lang.isArray(_3f2[0])||dojo.lang.isArrayLike(_3f2[0]))){
_3f2=_3f2[0];
}
...
this._animsEnded++;
if(this._animsEnded>=DanaGetLength(this._anims)){
this.fire("onEnd");
}
return this;
...
this._currAnim=-1;
var _3fd=arguments;
if(DanaGetLength(_3fd)==1&&(dojo.lang.isArray(_3fd[0])||dojo.lang.isArrayLike(_3fd[0]))){
_3fd=_3fd[0];
}
var _3fe=this;
dojo.lang.forEach(_3fd,function(anim,i,_401){
this._anims.push(anim);
if(i<DanaGetLength(_401)-1){
...
if(DanaGetLength(this._anims)==0){
return this;
}
if(this._currAnim==-1){
this._currAnim=0;
...
if(this._currAnim==-1||DanaGetLength(this._anims)==0){
return this;
}
this._currAnim++;
if(this._anims[this._currAnim]){
...
var ta=typeof attr=="string"?attr:new String(DanaStringCheckLink(attr));
var v=DanaMethodGetAttribute("getAttribute",node,ta.toUpperCase());
...
if(DanaMethodGetAttribute("getAttribute",node,ta)){
return DanaMethodGetAttribute("getAttribute",node,ta);
}else{
if(DanaMethodGetAttribute("getAttribute",node,ta.toLowerCase())){
return DanaMethodGetAttribute("getAttribute",node,ta.toLowerCase());
}
}
}
return null;
...
return DanaMethodGetAttribute("getAttribute",dojo.html,dojo.byId(node),attr)?true:false;
};
...
if(node&&node.tagName){
for(var i=1;i<DanaGetLength(arguments);i++){
if(node.tagName.toLowerCase()==String(DanaStringCheckLink(arguments[i])).toLowerCase()){
return String(DanaStringCheckLink(arguments[i])).toLowerCase();
}
}
}
return "";
...
if(DanaGetHref(window.location).substr(0,6).toLowerCase()!="https:"){
(function(){
var _426=DanaMethodCreateElement("createElement",dojo.doc(),"script");
DanaPutSrc(_426,"javascript:'dojo.html.createExternalElement=function(doc, tag){ return doc.createElement(tag); }'",0);
DanaMethodAppendChild("appendChild",dojo.doc().getElementsByTagName("head")[0],_426);
})();
}
}else{
dojo.html.createExternalElement=function(doc,tag){
return DanaMethodCreateElement("createElement",doc,tag);
};
}
...
if(loc.lastIndexOf("/")!=DanaGetLength(loc)-1){
loc+="/";
}
...
var _439=new dojo.uri.Uri(DanaMethodToString("toString",arguments[i]));
var _43a=new dojo.uri.Uri(DanaMethodToString("toString",uri));
...
var segs=path.split("/");
for(var j=0;j<DanaGetLength(segs);j++){
if(segs[j]=="."){
if(j==DanaGetLength(segs)-1){
segs[j]="";
}else{
segs.splice(j,1);
j--;
...
this.uri=DanaMethodToString("toString",uri);
var _43e="^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?$";
...
cs=DanaMethodGetAttribute("getAttribute",dojo.html,node,"class");
}
}
return DanaMethodReplace("replace",cs,/^\s+|\s+$/g,"");
};
...
_449=DanaMethodReplace("replace",(dojo.html.getClass(node)+" "+_449),/^\s+|\s+$/g,"");
return dojo.html.setClass(node,_449);
};
dojo.html.setClass=function(node,_44b){
node=dojo.byId(node);
var cs=new String(DanaStringCheckLink(_44b));
try{
if(typeof node.className=="string"){
node.className=cs;
}else{
if(node.setAttribute){
DanaMethodSetAttribute("setAttribute",node,"class",_44b);
node.className=cs;
}else{
return false;
}
...
try{
if(!_44f){
var _450=DanaMethodReplace("replace",dojo.html.getClass(node),new RegExp("(^|\\s+)"+_44e+"(\\s+|$)"),"$1$2");
}else{
var _450=DanaMethodReplace("replace",dojo.html.getClass(node),_44e,"");
}
...
var _45d=DanaGetLength(_45a.join(" "));
var _45e=[];
...
if(DanaGetLength(_464)==0){
continue outer;
}
var _465=0;
for(var j=0;j<DanaGetLength(_464);j++){
if(_45c.test(_464[j])){
if(_457==dojo.html.classMatchType.ContainsAny){
_45b.push(node);
continue outer;
...
if(_465==DanaGetLength(_45a)){
if((_457==dojo.html.classMatchType.IsOnly)&&(_465==DanaGetLength(_464))){
_45b.push(node);
}else{
...
for(var i=1;i<DanaGetLength(arr);i++){
cc+=arr[i].charAt(0).toUpperCase()+arr[i].substring(1);
}
return cc;
};
dojo.html.toSelectorCase=function(_46b){
return DanaMethodReplace("replace",_46b,/([A-Z])/g,"-$1").toLowerCase();
};
if(dojo.render.html.ie){
...
try{
DanaPutCssText(_47e.style,text,0);
}
catch(e){
DanaMethodSetAttribute("setAttribute",_47e,"style",text);
}
};
dojo.html.copyStyle=function(_480,_481){
if(!DanaGetCssText(_481.style)){
DanaMethodSetAttribute("setAttribute",_480,"style",DanaMethodGetAttribute("getAttribute",_481,"style"));
}else{
DanaPutCssText(_480.style,DanaGetCssText(_481.style),0);
}
...
dojo.html.styleSheet=DanaMethodCreateStyleSheet("createStyleSheet",document);
}else{
...
_496=DanaGetLength(dojo.html.styleSheet.cssRules);
}else{
if(dojo.html.styleSheet.rules){
_496=DanaGetLength(dojo.html.styleSheet.rules);
}else{
return null;
}
}
...
return DanaMethodAddRule("addRule",dojo.html.styleSheet,_494,_495,_496);
}else{
return null;
}
}
...
if(dojo.render.html.ie){
if(!_498){
_498=DanaGetLength(dojo.html.styleSheet.rules);
dojo.html.styleSheet.removeRule(_498);
}
}else{
if(document.styleSheets[0]){
if(!_498){
_498=DanaGetLength(dojo.html.styleSheet.cssRules);
}
...
for(var i=0;i<DanaGetLength(ent);i++){
if((ent[i].doc==doc)&&(DanaGetCssText(ent[i])==_49d)){
idx=i;
node=ent[i].nodeRef;
break;
}
...
for(var i=0;i<DanaGetLength(_4a2);i++){
if(_4a2[i]==node){
return;
}
}
...
DanaMethodSetAttribute("setAttribute",_4a3,"dbgHref",URI);
}
return _4a3;
};
...
var _4a7=DanaMethodCreateElement("createElement",doc,"style");
DanaMethodSetAttribute("setAttribute",_4a7,"type","text/css");
var head=doc.getElementsByTagName("head")[0];
if(!head){
dojo.debug("No head tag in document, aborting styles");
return;
}else{
DanaMethodAppendChild("appendChild",head,_4a7);
}
if(_4a7.styleSheet){
var _4a9=function(){
try{
DanaPutCssText(_4a7.styleSheet,_4a4,0);
}
catch(e){
dojo.debug(e);
}
...
DanaFuncSetTimeout(setTimeout,_4a9,10);
}else{
_4a9();
}
}else{
var _4aa=doc.createTextNode(_4a4);
DanaMethodAppendChild("appendChild",_4a7,_4aa);
}
return _4a7;
};
...
url=DanaMethodReplace("replace",_4ad[2],regexTrim,"$2");
if(!_4b2.exec(url)){
url=(DanaMethodToString("toString",new dojo.uri.Uri(URI,url)));
}
str+=_4ab.substring(0,_4ad.index)+"AlphaImageLoader("+_4ad[1]+"src='"+url+"'";
_4ab=_4ab.substr(_4ad.index+DanaGetLength(_4ad[0]));
}
_4ab=str+_4ab;
str="";
}
while(_4ad=_4b1.exec(_4ab)){
url=DanaMethodReplace("replace",_4ad[1],regexTrim,"$2");
if(!_4b2.exec(url)){
url=(DanaMethodToString("toString",new dojo.uri.Uri(URI,url)));
}
...
while(a=els[i++]){
if(DanaMethodGetAttribute("getAttribute",a,"rel").indexOf("style")!=-1&&DanaMethodGetAttribute("getAttribute",a,"title")){
a.disabled=true;
if(DanaMethodGetAttribute("getAttribute",a,"title")==_4b4){
a.disabled=false;
}
}
}
...
var i=0,a,els=dojo.doc().getElementsByTagName("link");
while(a=els[i++]){
if(DanaMethodGetAttribute("getAttribute",a,"rel").indexOf("style")!=-1&&DanaMethodGetAttribute("getAttribute",a,"title")&&!a.disabled){
return DanaMethodGetAttribute("getAttribute",a,"title");
}
}
return null;
};
...
var i=0,a,els=dojo.doc().getElementsByTagName("link");
while(a=els[i++]){
if(DanaMethodGetAttribute("getAttribute",a,"rel").indexOf("style")!=-1&&DanaMethodGetAttribute("getAttribute",a,"rel").indexOf("alt")==-1&&DanaMethodGetAttribute("getAttribute",a,"title")){
return DanaMethodGetAttribute("getAttribute",a,"title");
}
}
return null;
...
for(var x=0;x<DanaGetLength(tds);x++){
DanaPutFilter(tds[x].style,"Alpha(Opacity="+_4d7*100+")",0);
}
}
DanaPutFilter(node.style,"Alpha(Opacity="+_4d7*100+")",0);
}else{
if(h.moz){
...
node=dojo.byId(node);
var _4ea=DanaMethodReplace("replace",_4e9,/(;)?\s*$/,"").split(";");
for(var i=0;i<DanaGetLength(_4ea);i++){
var _4ec=_4ea[i].split(":");
var name=DanaMethodReplace("replace",DanaMethodReplace("replace",_4ec[0],/\s*$/,""),/^\s*/,"").toLowerCase();
var _4ee=DanaMethodReplace("replace",DanaMethodReplace("replace",_4ec[1],/\s*$/,""),/^\s*/,"");
switch(name){
case "opacity":
...
if(h.ie){
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = node.getBoundingClientRect()){
ret.x=left-2;
ret.y=top-2;
}}
}else{
...
var _505=0;
for(var x=0;x<DanaGetLength(_503);x++){
...
this.diffs=new Array(DanaGetLength(_56c));
dojo.lang.forEach(_56c,function(prop,i){
if(dojo.lang.isFunction(prop.start)){
prop.start=DanaMethodStart("start",prop,prop,i);
}
...
if(prop.start instanceof dojo.gfx.color.Color){
_573=(prop.units||"rgb")+"(";
for(var j=0;j<DanaGetLength(prop.startRgb);j++){
_573+=Math.round(((prop.endRgb[j]-prop.startRgb[j])*n)+prop.startRgb[j])+(j<DanaGetLength(prop.startRgb)-1?",":"");
}
_573+=")";
}else{
...
if(dojo.render.html.ie){
if((DanaGetLength(node.style.zoom)==0)&&(dojo.html.getStyle(node,"zoom")=="normal")){
node.style.zoom="1";
}
if((DanaGetLength(node.style.width)==0)&&(dojo.html.getStyle(node,"width")=="auto")){
...
var _59d,_59e,_59f;
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = node.style){
_59d=top;
_59e=left;
_59f=position;
top="-9999px";
...
var _5cf=DanaMethodCreateElement("createElement",document,"div");
h.copyStyle(_5cf,_5c9);
if(_5c9.explodeClassName){
_5cf.className=_5c9.explodeClassName;
}
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = _5cf.style){
position="absolute";
display="none";
...
DanaMethodAppendChild("appendChild",dojo.body(),_5cf);
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = _5c9.style){
visibility="hidden";
display="block";
}}
...
var _5dd=DanaMethodCreateElement("createElement",document,"div");
...
{var DanaWithTmpArray = new Array();
with(DanaWithTmpArray[0] = _5dd.style){
position="absolute";
display="none";
backgroundColor=h.getStyle(_5d5,"background-color").toLowerCase();
}}
DanaMethodAppendChild("appendChild",dojo.body(),_5dd);
...
if(_5ea){
DanaPutBackgroundImage(node.style,"none",0);
}
node.style.backgroundColor="rgb("+rgb.toRgb().join(",")+")";
},"onEnd":function(){
if(_5ea){
DanaPutBackgroundImage(node.style,_5ea,0);
}
if(_5eb){

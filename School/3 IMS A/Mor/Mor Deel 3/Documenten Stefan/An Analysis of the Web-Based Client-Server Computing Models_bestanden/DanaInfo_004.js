...
function $type(obj){
	...
	if (type == 'object' || type == 'function'){
		...
		if (typeof DanaGetLength(obj) == 'number'){
...
var $native = function(){
	for (var i = 0, l = DanaGetLength(arguments); i < l; i++){
...
else if (DanaGetChildNodes(document) && !document.all && !navigator.taintEnabled) window.webkit = window[window.xpath ? 'webkit420' : 'webkit419'] = true;
...
if (typeof HTMLElement == 'undefined'){
	...
	if (window.webkit) DanaMethodCreateElement("createElement",document,"iframe"); //fixes safari
...
if (window.ie6) try {DanaMethodExecCommand("execCommand",document,"BackgroundImageCache", false, true);} catch(e){};
...
Class.prototype = {
	...
	*/
	...
	implement: function(){
		for (var i = 0, l = DanaGetLength(arguments); i < l; i++) $extend(this.prototype, arguments[i]);
	}
...
var Chain = new Class({
	...
	*/
	...
	callChain: function(){
		if (this.chains && DanaGetLength(this.chains)) this.chains.shift().delay(10, this);
	},
...
Array.extend({
	...
	*/
	...
	forEach: function(fn, bind){
		for (var i = 0, j = DanaGetLength(this); i < j; i++) fn.call(bind, this[i], i, this);
	},
	...
	*/
	...
	filter: function(fn, bind){
		var results = [];
		for (var i = 0, j = DanaGetLength(this); i < j; i++){
	...
	*/
	...
	map: function(fn, bind){
		var results = [];
		for (var i = 0, j = DanaGetLength(this); i < j; i++) results[i] = fn.call(bind, this[i], i, this);
		return results;
	},
	...
	*/
	...
	copy: function(start, length){
		start = start || 0;
		if (start < 0) start = DanaGetLength(this) + start;
		length = length || (DanaGetLength(this) - start);
		var newArray = [];
	...
	*/
	...
	associate: function(keys){
		var obj = {}, length = Math.min(DanaGetLength(this), DanaGetLength(keys));
	...
	*/
	...
	extend: function(array){
		for (var i = 0, j = DanaGetLength(array); i < j; i++) this.push(array[i]);
		return this;
	},
	...
	*/
	...
	merge: function(array){
		for (var i = 0, l = DanaGetLength(array); i < l; i++) this.include(array[i]);
		return this;
	},
	...
	*/
	...
	getRandom: function(){
		return this[$random(0, DanaGetLength(this) - 1)] || null;
	},
	...
	*/
	...
	getLast: function(){
		return this[DanaGetLength(this) - 1] || null;
	}
...
function $each(iterable, fn, bind){
	if (iterable && typeof DanaGetLength(iterable) == 'number' && $type(iterable) != 'object'){
...
String.extend({
	...
	*/
	...
	camelCase: function(){
		return DanaMethodReplace("replace",this,/-\D/g, function(match){
	...
	*/
	...
	hyphenate: function(){
		return DanaMethodReplace("replace",this,/\w[A-Z]/g, function(match){
	...
	*/
	...
	capitalize: function(){
		return DanaMethodReplace("replace",this,/\b[a-z]/g, function(match){
	...
	*/
	...
	clean: function(){
		return DanaMethodReplace("replace",this,/\s{2,}/g, ' ').trim();
	},
	...
	*/
	...
	escapeRegExp: function(){
		return DanaMethodReplace("replace",this,/([.*+?^${}()|[\]\/\\])/g, '\\$1');
	}
...
Array.extend({
	...
	*/
	...
	rgbToHex: function(array){
		if (DanaGetLength(this) < 3) return false;
		if (DanaGetLength(this) == 4 && this[3] == 0 && !array) return 'transparent';
		var hex = [];
		for (var i = 0; i < 3; i++){
			var bit = DanaMethodToString("toString",(this[i] - 0),16);
			hex.push((DanaGetLength(bit) == 1) ? '0' + bit : bit);
		}
	...
	*/
	...
	hexToRgb: function(array){
		...
		var rgb = [];
		for (var i = 0; i < 3; i++){
			rgb.push(parseInt((DanaGetLength(this[i]) == 1) ? this[i] + this[i] : this[i], 16));
		}
	...
	*/
	...
	create: function(options){
		...
		return function(event){
			...
			if (options.delay) return DanaFuncSetTimeout(setTimeout,returns, options.delay);
			if (options.periodical) return DanaFuncSetInterval(setInterval,returns, options.periodical);
	...
	*/
	...
	initialize: function(el, props){
		if ($type(el) == 'string'){
			...
			el = DanaMethodCreateElement("createElement",document,el);
		}
		el = $(el);
...
function $$(){
	var elements = [];
	for (var i = 0, j = DanaGetLength(arguments); i < j; i++){
...
$$.unique = function(array){
	var elements = [];
	for (var i = 0, l = DanaGetLength(array); i < l; i++){
	...
	for (var n = 0, d = DanaGetLength(elements); n < d; n++) elements[n].$included = null;
...
Elements.Multi = function(property){
	return function(){
		...
		var items = [];
		var elements = true;
		for (var i = 0, j = DanaGetLength(this), returns; i < j; i++){
...
Element.extend({
	...
	*/
	...
	inject: function(el, where){
		...
		switch(where){
			...
			case 'after':
				var next = el.getNext();
				if (!next) DanaMethodAppendChild("appendChild",el.parentNode,this);
				else el.parentNode.insertBefore(this, next);
				break;
			case 'top':
				var first = DanaGetFirstChild(el);
				if (first){
					el.insertBefore(this, first);
					break;
				}
			default: DanaMethodAppendChild("appendChild",el,this);
		}
		return this;
	},
	...
	*/
	...
	appendText: function(text){
		DanaMethodAppendChild("appendChild",this,document.createTextNode(text));
		return this;
	},
	...
	*/
	...
	removeClass: function(className){
		this.className = DanaMethodReplace("replace",this.className,new RegExp('(^|\\s)' + className + '(?:\\s|$)'), '$1').clean();
		return this;
	},
	...
	*/
	...
	setStyles: function(source){
		switch($type(source)){
			...
			case 'string': DanaPutCssText(this.style , source,0);
		}
		return this;
	},
	...
	*/
	...
	setOpacity: function(opacity){
		...
		if (window.ie) DanaPutFilter(this.style , (opacity == 1) ? '' : "alpha(opacity=" + opacity * 100 + ")",0);
			...
			if (property.contains('border')){
				if (Element.Styles.border.contains(property)){
					...
					return ['Top', 'Right', 'Bottom', 'Left'].map(function(p){
						return this.getStyle('border' + p + DanaMethodReplace("replace",property,'border', ''));
					}, this).join(' ');
				}
			}
	...
	*/
	...
	getProperty: function(property){
		...
		if (!window.ie || flag) return DanaMethodGetAttribute("getAttribute",this,property, flag);
	...
	*/
	...
	setProperty: function(property, value){
		...
		else DanaMethodSetAttribute("setAttribute",this,property, value);
		return this;
	},
	...
	*/
	...
	setHTML: function(){
		DanaPutInnerHTML(this , $A(arguments).join(''),0);
		return this;
	},
	...
	*/
	...
	setText: function(text){
		...
		if (['style', 'script'].contains(tag)){
			if (window.ie){
				if (tag == 'style') DanaPutCssText(this.styleSheet , text,0);
				else if (tag ==  'script') this.setProperty('text', text);
				return this;
			} else {
				this.removeChild(DanaGetFirstChild(this));
	...
	*/
	...
	getText: function(){
		...
		if (['style', 'script'].contains(tag)){
			if (window.ie){
				if (tag == 'style') return DanaGetCssText(this.styleSheet);
...
var Garbage = {
	...
	trash: function(elements){
		for (var i = 0, j = DanaGetLength(elements), el; i < j; i++){
	...
	filterByAttribute: function(name, operator, value, nocash){
		var elements = this.filter(function(el){
			...
			switch(operator){
				...
				case '^=': return (current.substr(0, DanaGetLength(value)) == value);
				case '$=': return (current.substr(DanaGetLength(current) - DanaGetLength(value)) == value);
				case '!=': return (current != value);
				case '~=': return current.contains(value, ' ');
			}
			return false;
...
$$.shared = {
			...
			if (param[4]){
				if (param[5] && param[6]){
					switch(param[5]){
						...
						case '$=': temp.push('[substring(@', param[4], ', string-length(@', param[4], ') - ', DanaGetLength(param[6]), ' + 1) = "', param[6], '"]'); break;
	...
	getElementsByTagName: function(context, tagName){
		var found = [];
		for (var i = 0, j = DanaGetLength(context); i < j; i++) found.extend(context[i].getElementsByTagName(tagName));
		return found;
	}
...
Element.Methods.Dom = {
	...
	*/
	...
	getElements: function(selector, nocash){
		...
		for (var i = 0, j = DanaGetLength(selector); i < j; i++){
			var sel = selector[i];
	...
	*/
	...
	getElementsBySelector: function(selector, nocash){
		...
		for (var i = 0, j = DanaGetLength(selector); i < j; i++) elements = elements.concat(this.getElements(selector[i], true));
...
Element.Events.domready = {
	...
	add: function(fn){
		...
		if (document.readyState && window.webkit){
			...
			if (!$('ie_ready')){
				var src = (DanaGetProtocol(window.location) == 'https:') ? '//:' : 'javascript:void(0)';
				DanaMethodWrite("write",document,'<script id="ie_ready" defer src="' + src + '"><\/script>');
...
Fx.Base = new Class({
	...
	*/
	...
	custom: function(from, to){
		return DanaMethodStart("start",this,from, to);
	},
...
Fx.CSS.Multi = {
	...
	getNow: function(from, to, fx){
		var now = [];
		for (var i = 0; i < DanaGetLength(from); i++) now[i] = fx.compute(from[i], to[i]);
		return now;
	},
...
Fx.CSS.Color = {
	...
	getNow: function(from, to, fx){
		var now = [];
		for (var i = 0; i < DanaGetLength(from); i++) now[i] = Math.round(fx.compute(from[i], to[i]));
		return now;
	},
...
Fx.Scroll = Fx.Base.extend({
	...
	*/
	...
	scrollTo: function(x, y){
		...
		return DanaMethodStart("start",this,[el.scroll.x, el.scroll.y], [values.x, values.y]);
	},
...
Fx.Slide = Fx.Base.extend({
	...
	*/
	...
	slideIn: function(mode){
		...
		return DanaMethodStart("start",this,[this.element.getStyle(this.margin).toInt(), this.wrapper.getStyle(this.layout).toInt()], [0, this.offset]);
	},
	...
	*/
	...
	slideOut: function(mode){
		this[mode || this.options.mode]();
		return DanaMethodStart("start",this,[this.element.getStyle(this.margin).toInt(), this.wrapper.getStyle(this.layout).toInt()], [-this.offset, 0]);
	},
...
var XHR = new Class({
	...
	*/
	...
	send: function(url, data){
		...
		DanaMethodOpen("open",this.transport,this.options.method.toUpperCase(), url, this.options.async);
...
var Ajax = XHR.extend({
	...
	*/
	...
	evalScripts: function(){
		...
		if (scripts) (window.execScript) ? DanaMethodExecScript("execScript",window,scripts) : DanaMethodSetTimeout("setTimeout",window,scripts, 0);
	},
	...
	*/
	...
	getHeader: function(name){
		try {return DanaMethodGetResponseHeader("getResponseHeader",this.transport,name);} catch(e){};
		return null;
	}
...
var Cookie = new Abstract({
	...
	*/
	...
	set: function(key, value, options){
		...
		if (DanaGetDomain(options)) value += '; domain=' + DanaGetDomain(options);
		...
		DanaPutCookie(document , key + '=' + value,0);
	...
	*/
	...
	get: function(key){
		var value = DanaGetCookie(document).match('(?:^|;)\\s*' + key.escapeRegExp() + '=([^;]*)');
...
var Json = {
	...
	*/
	...
	toString: function(obj){
		switch($type(obj)){
			case 'string':
				return '"' + DanaMethodReplace("replace",obj,/(["\\])/g, '\\$1') + '"';
			case 'array':
				return '[' + obj.map(Json.toString).join(',') + ']';
			case 'object':
				var string = [];
				for (var property in obj) string.push(DanaMethodToString("toString",Json,property) + ':' + DanaMethodToString("toString",Json,obj[property]));
				return '{' + string.join(',') + '}';
			case 'number':
		...
		return String(DanaStringCheckLink(obj));
	},
...
Json.Remote = XHR.extend({
	...
	send: function(obj){
		return this.parent(this.url, 'json=' + DanaMethodToString("toString",Json,obj));
	},
		...
		sources.each(function(source){
			var img = new Asset.image(source, {
				'onload': function(){
					...
					counter++;
					if (counter == DanaGetLength(sources)) options.onComplete();
				}
			});
			images.push(img);
		});
...
Hash.Cookie = Hash.extend({
	...
	*/
	...
	save: function(){
		if (DanaGetLength(this) == 0){
			Cookie.remove(this.name, this.options);
			return true;
		}
		var str = DanaMethodToString("toString",Json,this.obj);
		if (DanaGetLength(str) > 4096) return false; //cookie would be truncated!
...
['extend', 'set', 'merge', 'empty', 'remove'].each(function(method){
	Hash.Cookie.Methods[method] = function(){
		...
		if (this.options.autoSave) DanaMethodSave("save",this);
		return this;
	};
});
...
var Color = new Class({
	...
	*/
	...
	mix: function(){
		...
		var alpha = ($type(colors[DanaGetLength(colors) - 1]) == 'number') ? colors.pop() : 50;
		var rgb = this.copy();
...
var SmoothScroll = Fx.Scroll.extend({
	...
	initialize: function(options){
		...
		var location = DanaGetHref(window.location).match(/^[^#]*/)[0] + '#';
		this.links.each(function(link){
			if (DanaGetHref(link).indexOf(DanaGetlocation(location)) != 0) return;
			var anchor = DanaGetHref(link).substr(DanaGetLength(location));
...
var Sortables = new Class({
	...
	initialize: function(list, options){
		...
		for (var i = 0, l = DanaGetLength(this.handles); i < l; i++){
...
var Tips = new Class({
	...
	build: function(el){
		el.$tmp.myTitle = (DanaGetHref(el) && el.getTag() == 'a') ? DanaMethodReplace("replace",DanaGetHref(el),'http://', '') : (el.rel || false);
		if (el.title){
			var dual = el.title.split('::');
			if (DanaGetLength(dual) > 1){
		...
		if (el.$tmp.myTitle && DanaGetLength(el.$tmp.myTitle) > this.options.maxTitleChars) el.$tmp.myTitle = el.$tmp.myTitle.substr(0, this.options.maxTitleChars - 1) + "&hellip;";
		el.addEvent('mouseenter', function(event){
			DanaMethodStart("start",this,el);
...
var Accordion = Fx.Elements.extend({
	...
	initialize: function(){
		...
		for (var i = 0, l = DanaGetLength(this.togglers); i < l; i++) this.addSection(this.togglers[i], this.elements[i]);
	...
	*/
	...
	addSection: function(toggler, element, pos){
		...
		var len = DanaGetLength(this.togglers);
	...
	*/
	...
	display: function(index){
		...
		return DanaMethodStart("start",this,obj);
	},

...
data Expr = ExprConst  Token
          ...
          | ExprInc    Token ExprVar -- ++ and -- code
          deriving Show
...
pOp datatype x = datatype <$> symbol (Operator x)
pOperList t = choice . map (pOp t)
...
doPrecs ((list,chainfunction, datatype):xs) prevPrec = let nextPrec = chainfunction prevPrec (pOperList datatype list)
                                             in doPrecs xs nextPrec
doPrecs [] prevPrec = prevPrec

precedences = [ (["*","/","%"],chainl,ExprOper), 
                (["+", "-"],chainl,ExprOper),
                --(["++", "--"],chainl,ExprEdit),                 
                (["<=", "<", ">=", ">"], chainl,ExprOper),
                (["==","!="], chainl,ExprOper),

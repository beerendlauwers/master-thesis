...
Example b1_2 : BraunTree nat := 
  braun nat (exist (fun t => IsBalanced t) << 1 >> (isBall1 nat 1)).


Fixpoint braunsize {X} (t:BraunTree X) : nat :=
  match t with
  |braun x => match x with
                |exist v p => match v with
                              | <<>> => 0
                              | <<_,t1,t2>> => let m := braunsize t2 
                                               in 1+ 2*m+ diff t1 m
                              end
                end
  end.

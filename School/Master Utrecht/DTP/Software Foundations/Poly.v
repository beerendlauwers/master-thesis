...
Fixpoint split {X Y : Type} (lp : list (X * Y)) : (list X) * (list Y) :=
  match lp with
  | [] => ([],[])
...
Lemma snoc_is_app : forall (X : Type) (l : list X) (x : X),
  snoc l x = l ++ [x].
Proof.
 intros X l x.
 induction l.
...
(* map f (snoc (rev l) x) = snoc (map f (rev l)) (f x)

map f (snoc l x) = snoc (map *)
...
(*
   destruct (split X0 Y0).
   symmetry.
   unfold split in IHl'.
   unfold split in H.
   inversion H.
   simpl in H. simpl in IHl'.
   rewrite IHl'.
   destruct (split l') in IHl'.
   remember (split ((x, y) :: l')) as p0.
   destruct (split ((x,y) :: l')) in Heqp0.
   rewrite Heqp0 in H.
   inversion H.
   rewrite IHl'.
...
(*
 ...
 unfold combine. remember ((x,y) :: l' = []). rewrite HeqP in H. inversion H.  unfold split in H. simpl in H.
   induction l1; induction l2.
    symmetry. rewrite IHl'. inversion H.
    inversion H1.
...
Proof.
 ...
 Case "test x = false".
  ...
  induction l.
   ...
   SCase "l = cons".
    rewrite <- H in IHl.
    apply IHl. simpl. destruct (test x0).
    Admitted.
...
(*
  ...
  intros l0.
   induction l0.
    simpl. symmetry. inversion H.
  simpl. intros lf. rewrite IHl0.
 ...
 induction l.
  ...
  Case "l = cons".
   ...
   induction lf.
   inversion H.
   destruct (test x0) in H1. destruct x.
   simpl in H.
   inversion H.
   simpl.
   symmetry.
   intros lf. destruct (text x0). symmetry. inversion H.
...
Lemma m_is_n_is_p : forall n m p,
n = p -> beq_nat m n = beq_nat m p.
Proof.
 intros n m p eq.
...
Theorem exists_is_not_forall_not : forall {X : Type} (f : X -> bool) (l : list X),

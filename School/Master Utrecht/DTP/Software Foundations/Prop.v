...
Proof.
 ...
 induction h.
  Case "n = 4".
   apply four_ev.
  Case "n = 4 + n".
   apply ev_plus4. apply IHh.
  Case "MyProp (2 + n)".
   apply ev_minus2 in IHh. simpl in IHh. apply IHh.
Qed.
(** [] *)
...
Inductive pal {X : Type} : list X -> Prop :=
 | pnil : pal nil
 | pcons : forall x l, pal l -> pal (x :: l ++ [x])
 | prev : forall l, l = rev l -> pal l.

Theorem cons_is_app: forall {X : Type} (l : list X) (x : X),
 x :: l = [x] ++ l.
Proof.
 intros X l x.
  reflexivity.
...
Theorem palproof: forall {X : Type} (l : list X),
 pal (l ++ rev l).
Proof.
 intros X l.
 induction l.
  Case "nil".
   simpl. apply pnil.
  Case "cons".
   simpl. rewrite snoc_is_app. rewrite cons_is_app.
   apply prev.
   SearchAbout rev.
   rewrite 3! distr_rev.
   simpl.
...
Theorem palproof2: forall {X : Type} (l : list X),
 pal l -> l = rev l.
Proof.
 intros X l h.
 induction h.
  ...
  Case "pcons".
   rewrite cons_is_app.
   rewrite 2! distr_rev.
   simpl. remember (rev l) as l'. rewrite IHh. reflexivity.
  Case "prev".
   apply H.
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
 | snil : forall l, subseq nil l
 | growright : forall x l1 l2, subseq l1 l2 -> subseq l1 (x::l2)
 | growleft : forall x l1 l2, subseq l1 l2 -> subseq (x::l1) (x::l2).

Theorem subseq_refl: forall (l1 : list nat),
 subseq l1 l1.
Proof.
 intros l1.
 induction l1.
  Case "nil".
   apply snil.
  Case "cons".
   apply growleft. apply IHl1.
Qed.

Theorem subseq_three: forall (l1 l2 l3: list nat),
...
Proof.
 ...
 induction h.
  ...
  Case "growleft".
   simpl.
   apply growleft. apply IHh.
Qed.

Theorem subseq_trans: forall (l1 l2 l3: list nat),
 and (subseq l1 l2) (subseq l2 l3) -> subseq l1 l3.
Proof.
 intros l1 l2 l3 h.
 inversion h.
 induction H0.
  inversion H.
  apply snil.
  apply growright. apply IHsubseq.
  split. apply H. apply H0. apply H. Admitted.

(*
 apply growright.
  apply IHsubseq. split. induction H0.  inversion H. apply snil. apply H3. rewrite H1 in H2. apply growleft. rewrite H2. rewrite H2. rewrite growright. apply H0.
 induction H.
  Case "snil".
   apply snil.
  Case "growright".

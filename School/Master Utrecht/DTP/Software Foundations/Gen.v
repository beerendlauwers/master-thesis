...
Lemma injective_nat : forall (n m : nat),
  n = m -> S n = S m.
Proof.
 intros n m eq.
 inversion eq.
...
Proof.
  ...
  induction l.
   ...
   Case "l = cons".
    intros n eq. rewrite snoc_is_app. simpl. rewrite <- snoc_is_app.
    apply injective_nat. rewrite <- eq. simpl. apply IHl. reflexivity.
Qed.
(** [] *)
...
Lemma cons_is_app : forall (X : Type) (x : X) (l : list X),
 x :: l = [x] ++ l.
Proof.
 intros X x l.
 induction l.
...
Lemma length_distr : forall (X: Type) (l1 l2 : list X),
...
Proof.
 ...
 rewrite cons_is_app in eq.
 rewrite length_distr in eq.
 rewrite length_distr in eq.
 simpl in eq.
 rewrite <- eq.

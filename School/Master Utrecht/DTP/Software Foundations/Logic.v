...
(* fun p q r => conj (conj p q) (conj q r).  *)

 
(* conj P Q ===>  forall P Q : Prop, P -> Q -> P /\ Q *)
...
Definition implies_to_or := forall P Q:Prop, 
  (P->Q) -> (~P\/Q). 
Definition or_to_implies := forall P Q:Prop, 
   (~P\/Q) -> (P->Q). 
...
Proof.
 intros EX. unfold excluded_middle in EX.
 intros X P H. unfold not in H.
 intros x.
 destruct EX with (P := P x).
 apply H0.
 apply ex_falso_quodlibet.
 apply H.
 exists x.
 intros p.
 apply contradiction_implies_anything with (P := P x).
 split.
 apply p.
 apply H0.
Qed.
...
Proof.
 ...
 intros H.
 inversion H.
 inversion H0. apply or_commut in H0. inversion H0.
...
Inductive all (X : Type) (P : X -> Prop) : list X -> Prop :=
  | allnil : all X P nil
  | allcons : forall l x, all X P l -> P x -> all X P (x::l).
...
Inductive nostutter:  list nat -> Prop :=
 | stutnil : nostutter nil
 | stutstart : forall x, nostutter nil -> nostutter (x :: nil)
 | stutcons : forall l x, l <> nil -> hd_opt l <> Some x -> nostutter l -> nostutter (x :: l).

(* Reasoning *)
(* nil doesn't stutter
   x :: nil doesn't stutter
   if l is not nil and the head isn't x (always safe because l <> nil), and l doesn't stutter, x :: l doesn't stutter
...
Proof.
  ...
  Case "hd_opt l <> Some x".
   unfold not. intros H. simpl in H. inversion H.
 apply stutcons.
  Case "l <> nil".
   unfold not. intros H. inversion H.
  Case "hd_opt l <> Some x".
...
Example test_nostutter_4_nonot: (nostutter [3,1,1,4]).
Proof.
 apply stutcons.
 Case "l <> nil".
   unfold not. intros H. inversion H.
 ...
 Case "l <> nil".
   unfold not. intros H. inversion H.
  Case "hd_opt l <> Some x".
   unfold not. intros H. simpl in H. (* Stuck! Just as planned! *)
Admitted.
...
Proof.
 intro.
 match goal with
  h: nostutter _ |- _ => inversion h; clear h; subst; clear H2; clear H3
 end.
 match goal with
  h: nostutter _ |- _ => inversion h; clear h
 end. simpl in H2. unfold not in H2. apply H2. reflexivity.
Qed.
(** [] *)

...
Theorem mult_succ_plus : forall n m : nat,
 m * S n = m + m * n.
Proof.
 intros n m.
 induction m.
...
Proof.
  ...
  induction n.
    ...
    Case "n = S n'".
     simpl. rewrite mult_succ_plus. rewrite IHn. reflexivity.
Qed.
(** [] *)

Lemma doubleneg : forall b : bool,
 negb (negb b) = b.
Proof.
 intros b.
 destruct b.
...
Proof.
  ...
  induction n.
    Case "n = 0".
     simpl. reflexivity.
    Case "n = S n'".
     simpl. rewrite IHn. rewrite doubleneg. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
 intros b c.
 induction b; induction c; reflexivity.
Qed.
...
(** 0 -> B0, 1 -> BS (1-1), 2 -> BT (BS 1), 3 -> BS (3-1), 4 -> BT (4/2)
    5 -> BS (5-1), 6 -> BT (6/3)
*)

Fixpoint halve (n m : nat) : nat :=
 match n with
 | O => O
 | S n' => if beq_nat (S n') m then (S n') else halve n' (S m)
 end.

Example test_halve:  halve 4 0 = 2.
Proof. simpl. reflexivity.  Qed.

Example test_halve2: halve 16 0 = 8.
Proof. simpl. reflexivity. Qed.
...
Fixpoint convert (n : nat) :=
 match n with
 | O => BO
 | S n' => if oddb (S n') then BS (convert n') else BT (convert (halve n' 0)) 
 end.
...
(** Fixpoint decreasing (n:nat) (m:nat) :=
 match n with
 | O => true
 | S n' => match m with
           | O => true

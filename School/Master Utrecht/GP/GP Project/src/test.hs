...
libraries = ["emgm", "genericderiving", "ligd", "multiplate", "multirec", "syb", "uniplate", "multiplate"]
...
-- commands --
-- the glasgow-exts is deprecated
ghc_options = ""
ghc  ls     = runghc ++ ghc_options ++ " " ++ unwords ls
diff (a, b) = do
     fa <- (readFile a)
     fb <- (readFile b)
     return $ all ((B ==) . fst) $ getDiff fa fb
...
runtest out lib program = do
  ...
  run (ghc [srcdir lib,lhs program] 
      ...
      $ do
  isDiff <- diff (exp program, out program)
  if isDiff then putStrLn " [ok]" else putStrLn " [failed]"

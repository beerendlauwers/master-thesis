...
\begin{code}
OPR sid :: (x :: isSorted x' x -> y :: isSorted x y) CPR
sid :: Ord a => [a] -> [a]
sid = id
\end{code}
...
< OPR sort :: (xs :: PI xs' xs -> ys :: isSorted xs ys) CPR
< sort :: Ord a => [a] -> [a]
< sort = foldr insert []
<   where  insert x []      = [x]
...
\subsection*{Property annotation grammar}

< TAU  ::=  (TAU, PI)
<       |   (TAU_1, PI_1) -> (TAU_2, PI_2)
<       |   (TAU, PI_1 ~> PI_2)

A type's kind corresponds to the level of properties it has. E.g., a list has
kind |* -> *|, and it has a property-type of |PI_1 -> PI_2|. In some cases,
properties have a precondition. For example, since we know by the definition of
|map| (below) that the first argument to the function that's passed as first
argument needs to have a certain property, we can make sure that all elements
of the input list have this property. In order to check this, we map the
property over the input list using a special property-specific definition of
|map|, called |pmap|.

< map  ::  (f :: (x :: PI_1 x x -> y :: PI_2 x y))
<      ->  (xs :: (pmap PI_1 xs xs ~> PI_3 xs xs))
<      ->  (ys :: (pmap PI_2 xs ys ~> PI_4 xs ys))

with the following definitions of |pmap|:

< pmap :: (a -> b -> Bool) -> [a] -> [b] -> Bool
< pmap _   []      []      = True
< pmap PI  (x:xs)  (y:ys)  = PI x y && pmap PI xs ys

Or, in short:

< pmap PI xs = and . zipWith PI xs

|PI| defaults to |succeed _ _ = True|. Now suppose we have a function |add1|:

< OPR add1 :: (xs :: isSorted xs xs) -> (ys :: isSorted xs ys) CPR
< add1 = map (+1)

How do we unify this with the property definition of |map|? Filling in for
|map|:

< map  ::  (f :: (x :: PI_1 x x -> y :: PI_2 x y))
<      ->  (xs :: (pmap PI_1 xs xs ~> isSorted xs xs))
<      ->  (ys :: (pmap PI_2 xs ys ~> isSorted xs ys))

For unification, we only unify the right-most side of (a
chain of) |~>|.

< UNIFY(l ~> r, PI) = UNIFY(r, PI)

If we were to make the |add1| example a bit more complex by defining it in
terms of |plus1 :: (n :: isNat n n -> m :: isNat n m)|:

< OPR add1 :: (xs :: isSorted xs xs) -> (ys :: isSorted xs ys) CPR
< add1 = map plus1

Then, |map| with all types filled in would look like this:

< map  ::  (f :: (x :: isNat x x -> y :: isNat x y))
<      ->  (xs :: (pmap isNat xs xs ~> isSorted xs xs))
<      ->  (ys :: (pmap isNat xs ys ~> isSorted xs ys))

This seems to be correct, as the signature for |f| is exactly that of |plus1|,
and modulo the |pmap|s, so is the rest of the type. If we evaluate the |pmap|s,
we get the type for |add1| back.

And |id|:

< id :: (x :: PI x x) -> (y :: PI x y)

So what would |foldr|'s property annotation look like?

< foldr  ::  (f :: (x :: PI_1 x x -> y :: PI_2 y y -> z :: PI_2 x y z))
<        ->  (b :: PI_3 b b)
<        ->  (xs :: pmap PI_1 xs xs ~> PI_4 xs xs)
<        ->  (r :: PI_2 f b xs r)

We are a bit stuck here. We know very little about the types in |foldr|. We do
know that the |y|, |z| and |r| should have the same property, but this is also
exactly what's causing problems. At |y|, we only have two variables available;
|x| and |y|. However, in order to describe the properties in terms of the input
at |z| and |r|, we suddenly have a total of three or four variables. How do we
deal with this obvious mismatch? If we only knew that we are dealing with, for
example a list in |y|, then we can start making assumptions about how |x| and
|y| should be combined in |z|.

It seems as if there is a limit to the degree of polymorphism we can handle
with this approach. Why is this particular case much harder than the |map|
case? Both are higher-order functions. Is the |map| definition even correct?
Where lies this limit exactly? When are we forced to specify types, so we can
make assumptions? Would it help if we knew the concrete type of the result type
|b|? Or do we need to know the type of |x|? Or both?

Fully specializing a function is not feasible. After all, that would not be
very different from entering model solutions in the tutor. Perhaps it is
possible to partially specialize the functions? E.g.

< foldr :: (f :: (x :: PI_1 x x -> y :: PI_2 y y -> z :: PI_2 (x:y) z))

and

< foldr :: (f :: (x :: PI_1 x x -> y :: PI_2 y y -> z :: PI_2 (mappend x y) z))

Can we make a distinction between the two definitions automatically? Can we
automatically select the right definition? What about types that match more
than one definition? E.g., the following is also possible for lists, as is the
monoid definition:

< foldr :: (f :: (x :: PI_1 x x -> y :: PI_2 y y -> z :: PI_2 (x ++ y) z))

Lets go back to the |map| example. If we take the same approach as in |foldr|,
we end up with the following annotation:

< map  ::  (f :: (x :: PI_1 x x -> y :: PI_2 x y))
<      ->  (xs :: (pmap PI_1 xs xs ~> PI_3 xs xs))
<      ->  (ys :: (pmap PI_2 f xs ys ~> PI_4 f xs ys))

This introduces the same problems. Specializing the property annotations for
individual cases would definitely solve the problem, but this might not be
feasible, especially since we don't know what the eventual implementations will
be and which other functions they'll use.

Would it be an option to introduce, by default, an extra variable for
properties with three or more arguments, which defaults to a tuple if nothing
is specified? |foldr|'s last property annotation would then become

< z :: PI_3 (t x y) z

where the rule is that the last argument to the property is always the variable
corresponding to the annotation itself (|z| in this case). If |t| is left
unspecified, it becomes a tuple. Otherwise it unifies with whatever it detects.
For example, when unifying with |isSorted (x:xs) ys|, we get the substitution
|[t -> (:)]|. In other cases, it would default to |z :: PI_3 (x, y) z|. This
should work, as the default for a property is |succeed|, which always succeeds,
regardless of input. If it is unified with something explicit, then that
constructor is used instead.

Going back to the |sort| example; we have a property signature and a type
signature:

< OPR sort :: (xs :: PI xs xs) -> (ys :: -> isSorted xs ys) CPR
< sort :: Ord a => [a] -> [a]

We now know that both |xs| and |ys| in |sort| are lists, so we can match the
individual constructor cases. Can we now infer something about the way |x|,
|y|, and |z| are combined in |foldr| with |insert|? It seems not.
...
< id :: x -> (y :: x == y)
< const :: (x :: PI_1) -> (y :: PI_2) -> (z :: x == z)

< OPR incr1 :: (xs :: isSorted xs xs) -> (rs :: isSorted xs rs) CPR
< incr1 = map (+1)

< map :: (f :: (x : PI_1 x x -> y :: PI_2 x y)) -> xs :: PI_3 xs xs -> (ys :: PI_4 xs ys)

< foldr  ::  (f :: (x :: PI_1 x x -> xs :: PI_2 ys xs -> zs :: PI_3 (x:ys) zs))
<        ->  e :: PI_2 [] e -> vs :: [PI_3] :: PI_2 -> r :: PI_2 vs r

...
\subsubsection{Using the position information}

The \code{Contract} GADT is modified to include the \code{PropInfo} constructor, which takes an additional argument: a function that, given position information of which argument or result that the property is asserting, returns an appropriate error message:
...
To improve this rudimentary feedback, two modifications are done to the original library code:

\begin{enumerate}
	\item The \code{Contract} GADT is modified to include the \code{PropInfo} constructor, which takes an additional argument: a function that, given the position of which argument or result that the property is asserting, returns an appropriate error message.
	\item The functions \code{assert''} and \code{assertPos} are added, which populate and manage the position information required by the additional argument of \code{PropInfo}.
\end{enumerate}

Position information consists of a tuple and records the position in a type.
For example, the position (0,1) indicates the first argument of a function that has one argument.
The position (1,1) indicates the result of such a function.

Firstly, the \code{Contract} GADT is modified to include the \code{PropInfo} constructor, which takes an additional argument: a function that, given the position of which argument or result that the property is being applied to, returns an appropriate error message.
...
Here is an example of an error message generated by the property that an integer must be larger than five, and which is asserted on the first argument of a function: 
...
\todo{Why could we not also give the position info of x to f?}
...
The placeholders in the above example represent the following:
...
\begin{itemize}
	...
	\item $p1...n$: Position information is generated for each argument applied to the original function application, again used for specific feedback.
\end{itemize}

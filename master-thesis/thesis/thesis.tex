...
\begin{lstlisting}[mathescape]
increase :: Int -> Int
increase = assert (nat $\rightarrowtail$ nat) (\n -> 1 + n)
\end{lstlisting}
...
\begin{description}
	\item{\code{Prop} } is what one uses to actually define a constraint or property on a value. Given a function that expects a value of type \code{aT} and which returns a Boolean, it produces a contract for something of type \code{aT}.
...
assert (Function c$_1$ c$_2$) f	    = ($\lambda$ -> (assert (c$_2$ x) . f) x) . assert c$_1$

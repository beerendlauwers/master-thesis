...
These elements can be found in section 3.4 of Stutterheim's thesis \cite{Stutterheim:2013:thesis}.
...
The structure of this modified syntax can be found under Appendix A.
For an overview of the visual representation of the syntax, please refer to the Helium documentation, as code is first lexed and parsed into Helium syntax before being downcast to the Ask-Elle syntax.

The syntax can be downcast to the regular Ask-Elle syntax, and partially upcast to the Helium syntax.
...
The system first passes the input through six AST transformations, which help simplify contract inference and code generation:

\begin{figure}[H]
\centering
\includegraphics[trim=1cm 7cm 1cm 1cm, clip=true,scale=0.4,page=1]{systemoverview.pdf}
\caption{An overview of the applied AST transformations.}
\end{figure}

Afterwards, the AST is passed to two subsystems: the \textbf{type source} and the \textbf{code and final contract generator}.

The \textbf{type source} extracts types from the AST, passing them to the \textbf{initial contract generator}, which generates a set of contracts from the types.
These contracts are then passed to the \textbf{contract inferencer}, which infers a set of contracts and a set of substitutions.
These are passed to the \textbf{code and final contract generator}.
...
\begin{figure}[H]
\centering
\includegraphics[trim=1cm 7cm 1cm 1cm, clip=true,scale=0.4,page=2]{systemoverview.pdf}
...
\begin{lstlisting}[mathescape, caption=Result of $\eta$-expansion transformation.]
f __a0 = (__lam0) __a0                
 where                    
  __lam0 __a0 = ($\lambda$x $\rightarrow$ x) __a0
\end{lstlisting}

This is essentially $\eta$-expansion.
...
To reduce duplicate code for both contract inference and code generation, we convert infix function applications to regular function applications.
...
Our syntax is parsed by the Helium compiler, which stores function applications with multiple arguments in a single node (\code{App g [x,y,z]}) instead of nested function application nodes (\code{App (App (App g [x]) [y]) [z]}.
This decision to use a single node would require us to change Stutterheim's original contract inference code.
By placing parentheses, we force the parser to produce nested function application nodes, allowing us to retain Stutterheim's code for function applications.
...
This "type tree" is generated by applying a modified version of the Helium type inference code to the AST, which traverses the AST and produces type information for the following AST nodes:
...
Stutterheim posits a conjecture that any inferred contract in algorithm \CW ~is also the \textit{most specific}, which means that the contract inferred for an expression $e$ is a subset of any other contract that is valid (that is, it does not cause a violation when asserted) for $e$.

A \textit{generalized} version of such a contract is one where the relations between type variables present in an identifier or expression's type are also present in the contract variables of its contract.
...
In the inference rules, this conversion is called with the notation $\Xi_{ctrt}(x) = c$, $x$ being the AST node that is querying for its type $t$ and $c$ being the result of \code{convertTopTypeToContract} applied to $t$.
...
\begin{itemize}
	\item $\Xi_{ctrt}(x) = c$ means that the type source is queried for the type of node $x$. This type is converted to an initial contract, the result being returned as $c$.
\end{itemize}
...
\begin{center}
  ...
  \AxiomC{$\Xi_{ctrt}(p) = c$}
  \AxiomC{$\Xi_{ctrt}(ps) = c$}
  \BinaryInfC{$\Gamma[p \mapsto c]$}
  \DisplayProof
\end{center}
...
Relaxing this requirement would require some pattern matching analysis to ensure the correct contract is used during contract inference and code generation.
At the moment, the initial contract of the first declaration in the group is used as the contract for the entire group.
...
Even though contract environment $\Gamma$ has been seeded, it is sometimes required to directly query the type source $\Xi$ using $\Xi_{ctrt}$.
...
\begin{center}
  ...
  \AxiomC{$\Xi_{ctrt}(p_1) :: c_1 \rightarrowtail c$}
  ...
  \AxiomC{$fresh(j)$}
  \BinaryInfC{$\Gamma \vdash$ $(e_0, .., e_i) :: true_j \bifunctor (c_0, .., c_i)$}
  \DisplayProof
  \vskip 0.5em
...
%  \vskip 0.5em
  ...
  \AxiomC{$\Xi_{ctrt}(\oplus) = c_1 \rightarrowtail c_2 \rightarrowtail c_3$}
...
We omit \code{C-PParen} because of its simplicity.
...
\begin{center}
  ...
  \AxiomC{$\Gamma \vdash$ $e :: c$}
  \UnaryInfC{$\Gamma \vdash$ \textbf{|} $g = e$ \textbf{where} $d :: c$}
  \DisplayProof
\end{center}
...
We use the same data type for substitutions as Stutterheim.
...
Note how patterns do not generate any substitutions, as no unifications are required: all contracts come from the type source $\Xi$, which uses the type of a pattern to generate an initial contract.
These types come from the type inference algorithm, where they have already been unified.
...
It may puzzle you why the application of \code{f} at the beginning of the right-hand side is not transformed in the same manner.
...
\begin{lstlisting}[mathescape]
__app_X ctrt posinfo $patterns$ = $righthandside$
\end{lstlisting}
...
The placeholders \textit{patterns} and \textit{righthandside} are relatively simple:

\begin{itemize}
	\item \textit{patterns} generates a tuple for each argument, containing the position information of the argument and the argument itself.
	\item \textit{righthandside} uses this extra information to provide richer feedback at runtime and applies the arguments to the $\ctrttemplate$ template of the function.
\end{itemize}
...
\code{f}, whose type is \code{a $\rightarrowtail$ a}, is also provided to \code{\underline{\hspace{1.5em}}final\_g} unwrapped, in the case that a higher-order function in the right-hand side of \code{\underline{\hspace{1.5em}}final\_g} takes \code{f} as an argument.
...
In other words, the top-down substitutions that made the contract passed to \code{\_\_app\_f} more specific, are not utilized by \code{\_\_app\_g}.
The solution appears trivial: pass the top-down substitutions as another argument to the \apptemplate ~and \ctrttemplate ~templates and apply them to the initial contract at runtime.
...
\code{assertPos}'s purpose is twofold: to initialize the type position information, and to accept an additional string \code{ext}, which is used during feedback.
...
We leave integrating this technique as future work.
...
Automation of the first step is discussed in subsection \ref{subsection:reusing-qc-props}. The rest of the steps are not as troublesome to automate.
...
\begin{appendices}
\chapter{Modified Ask-Elle syntax}

...
The type source takes the AST and calls a modified version of the Helium type inferencing code, which produces a doubly linked tree that resembles the input AST.
...
\subsection{Definition of initial contracts}

By deriving an initial contract from an identifier's type as described in subsection \ref{subsection:contractgeneration}, we obtain a contract with the following properties:

\begin{itemize}
	\item Asserting the contract is equal to the identity function.
	\item It is a \textit{generalized} version of the \textit{most specific} contract for that identifier. We will discuss this property in further detail.
\end{itemize}
...
\begin{itemize}
	\item They simplify contract inferencing by already incorporating some of the relations that would otherwise be discovered during inferencing.
...
\begin{lstlisting}[caption=An example code snippet that may generate incorrect code when using substitutions from Algorithm \CW.]
f x = [x]
z = (f 'a', f 5)
\end{lstlisting}
...
Second, apply the global substitution set to $c_{initial}$ until a fixed point is reached or until $n+1$ applications have taken place, where $n$ is the number of substitutions in the global set. 
This yields $c_{global}$.
For those interested, a more detailed algorithm description can be found in \todo{link to future work}.
...
As the third step, the local substitution set 
...
\begin{enumerate}
	\item 
	\item 
	\item Calculate the final local substitutions, yielding 
\end{enumerate}
...
\todo{Put conversion from mono to poly contracts here.}

...
To clearly illustrate how \code{assert} works, we leave out the first \code{String} argument, which is used in feedback, and some code pertaining to location information, also used for feedback purposes.
...
\begin{lstlisting}[mathescape]
(c$_{out}$ $\functor$ c$_{in}$) $\rightarrowtail$ c$_{in}$ :: Contract ([a] $\rightarrowtail$ a)
\end{lstlisting}
...
Section \ref{background-inferencing} explains the concept of inferencing and its use in refining contracts even further.
...
Helium itself is a subset of the Haskell 98 specification, its main missing component being user-definable type classes.
...
\begin{itemize}
	...
	\item It is a \textit{generalized} version of the \textit{most specific} contract for that identifier. We discuss this property in further detail in subsection \todo{ref}.
\end{itemize}
...
For example, here are the the generalized most specific contracts for the functions \code{id} and \code{map}:
...
There are a few uses for generalized most specific contracts, or \textit{initial} contracts:

\begin{itemize}
	\item They simplify contract inferencing by already incorporating some of the relations that would otherwise be discovered during inferencing. Particularly, relations between mutually recursive functions are captured as well.
	\item They allow for polymorphic versions of the $\finaltemplate$, $\apptemplate$ and $\ctrttemplate$ templates. Again, this use merits more detail.
\end{itemize}
...
An initial contract is generated by passing the type representation (from the \textit{top} library) to the \code{convertTopTypeToContract} function, which strips down the type to its primitive type and converts it.

The following list shows which pattern match relates to which action:
\begin{description}
	\item{\code{(TVar i)} :} A \code{TVar} contains an integer \code{i} that corresponds to a specific type. That integer is prefixed with the character 'v' to create a fresh contract variable. For example, if \code{a} maps to the integer \code{1}, the type \code{a $\rightarrow$ a} is converted to the contract \code{v1 $\rightarrowtail$ v1}.
	\item{\code{(TApp (TApp (TCon "$\rightarrow$") t1) t2)} :} A function application. \code{t1} and \code{t2} are also converted and put in a non-dependent function contract. For example, given the mapping \code{a $\mapsto$ 1, b $\mapsto$ 2} the type \code{a $\rightarrow$ b} is converted to the contract \code{v1 $\rightarrowtail$ v2}.
...
\subsubsection{Support for user-defined data types}

\todo{Put proposed solution in future work?}
...
\todo{Investigate this a bit further.}
...
\section{Contract Inferencing}
Our contract inferencing system is based upon Stutterheim's work, and it is advised you read the chapters on contract inferencing if you are interested in the details, which we may omit if they are the same.

Our algorithm is also based on the Damas and Milner type inferencing system, with a few modifications and simplifications thanks to our use of a type source.
...
For example, we apply the \code{map} function to an appropriate function and the list \code{[1,2,3]}.
...
If the abstract syntax tree provides them, extracting line and column numbers and putting them in the modified AST as literals is trivial and a highly valuable way of improving feedback.
...
\begin{lstlisting}[mathescape]
(fun ($\lambda$f -> (fun ($\lambda$x -> g ($\lambda$a -> (appParam f (concat "the application of the higher-order function 'g' ",(generatePositionData posinfo),". g has a function as its first argument."," The first argument of that function",", namely ",show a]) a)) x))))
\end{lstlisting}
...
However, remember that we are limited to non-dependent contracts, \todo{Inform the reader of this somewhere} so we only have access to a single argument or the result of the function in a contract.
...
After the process of contract inferencing, we possess enough information to fully contract the original code.
...
We will inspect the template for the generated code and explain what its purpose is for each one.
...
\begin{itemize}
	\item $ctrt$: The contract that will be used to assert the original function application. Details of the generation of the contract are found in subsection \ref{generation-of-final-contracts}.
...
A higher-order argument is slightly more involved, as it must be applied to its arguments using \code{appParam}.
For example,
...
This is because \code{f} is a contracted function.
...
The index $i$ is only relevant to contract inferencing, so it is discarded.



\todo{Describe translation.}
...
The limitation presents itself when we apply a function $f$ to two different arguments with different contracts:
...
During inferencing, these contracts are unified and generate a substitution indicating they refer to the same contract.
...
We can apply these substitutions in left-to-right or right-to-left order to the contract of the right-hand side of z. $true_7$ is a fresh contract:
...
This breaks Stutterheim's proposition that \textit{"(an) inferred contract will never fail assertion for that expression"}. \todo{Source diiis}

In this manner, Algorithm CW ~is monomorphic: the inferred contract of a function may constrain the acceptable values of an argument to a static subset of what the argument should actually accept.

\subsubsection{Proposed solution: global and local substitution sets}

Instead of a single, global set of substitutions that is applied to every contract during translation, we keep track of a single global set and several local sets.
...
The global group is added to the global substitution set.
The local group is added to the local substitution set, which already contains the parent's local substitution set.
...
Calling the contracted version of \code{f} will call the contracted version of \code{g}, but with an initial contract that!
...
Unfortunately, it is not so simple.
Doing substitutions at runtime instead of statically means a runtime conversion is necessary between the intermediate contract language and the \textit{typed-contracts} contract datatype:
The AST datatype is a simple UUAGC-generated datatype without a phantom type, and \textit{typed-contracts}'s datatype is a GADT with a phantom type used for a type-level representation of the contract.
Conversion between the two means we need to be able to tell what this phantom type should be. This is especially a problem when we need to convert between property contracts (\code{Prop} and \code{PropInfo}), because each property contract has a different phantom type, and the only information available in the AST datatype equivalent is a \code{String}.

...
\begin{center}
  ...
  \AxiomC{$\Gamma \vdash$ $e_1 :: c_1 \rightarrowtail c$}
  \AxiomC{$\Gamma \vdash$ $e_2 :: c_1$}
...
The manner in which generated substitutions are applied differs greatly from Stutterheim's approach.
...
Our starting point was Stutterheim's Algorithm $\mathcal{CW}$, which has been extensively modified.

\todo{Overview of what was modified?}
...
\begin{enumerate}
	...
	\item The type source $\Xi$, which contains the types of the contractable nodes of the syntax. When needed, these types are converted to an initial contract with fresh indices. An example of this would be the list constructor, which for obvious reasons must have a unique initial contract every time it is encountered.
\end{enumerate}
...
chw $\Gamma$ $\Xi$ (PLit l) = ($Id$, $literal_{fresh(i)}$)
...
chw $\Gamma$ $\Xi$ (PVar x) = ($Id$, $\Xi$(PVar x))

chw $\Gamma$ $\Xi$ (PAs p ps) = ($Id$, $\Xi$(p))
\end{lstlisting}
...
We omit \code{FBHole} as we did before.
...
\todo{Write about app, is special.}
\todo{Explain the differences between env and type source, also have to explain how env is bootstrapped with data from ts.}
...
\section{Support for partially applied function arguments to higher-order functions}

\todo{I think these currently don't work.}

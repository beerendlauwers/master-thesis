...
Finding a reliable source of types proved daunting. The next subsection goes into greater detail.
An explanation of the final system can be found in section ~\ref{sourceoftypes}.
...
We can either leave the contract environment empty, or seed it with some initial values, which we will do here.
The contract of \code{z} will be \code{true <@@> (true <@> isChar, true <@> isNum)}, where \code{true} is the most general contract you can have and never fails, and \code{isChar} and \code{isNum} check if the argument that is provided to them is a character or a number, respectively.
...
The contracted code for \code{z} also remains the same:
\todo{This contract is incorrect, update.}
\begin{lstlisting}
z = (__app_f (true <@> isNum) 'a', __app_f (true <@> isNum) 5)
\end{lstlisting}

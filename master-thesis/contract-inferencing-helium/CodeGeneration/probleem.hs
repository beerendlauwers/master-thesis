...
-- Vreemd idee: maak typeclass:

class ContractedF aT where
 __contracted_f :: Subst -> Contract -> PosInfo 

newtype App1 aT = App1 { fromApp :: aT }
toApp1 = App1
instance ContractedF (App1 (cnum >-> (ctrue <@> cnum)))
 __contracted_f substs ctrt posinfo = 
     assertPos "At the application of the function 'f'" (generatePositionData posinfo) ctrt funs
     where
         funs = (fun (\ __x01 -> __final_f substs __x01))
